...
class HelperClass {
    ...
    static capitalCase(string) {
        return string.charAt(0).toUpperCase() + string.slice(1).toLowerCase();
    }
    static generateRandomChar(length = 32, type = "alpha") {
    ...
    static userNameValidator(string) {
        const strongRegex = new RegExp(/^[ A-Za-z0-9_-]*$/);
        if (!strongRegex.test(string)) {
            throw new Error("Invalid character in username. Only hiphen (-) and underscore (_) are allowed");
        }
    }
    static removeUnwantedProperties(object, properties) {
        let newObject = {};
    ...
    static isIdentifierEmailOrPhone(input) {
